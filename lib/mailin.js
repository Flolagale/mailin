...
var Mailin = function () {
    ...
    /* Exposing simplesmtp server instance. The instance will be initialized
       only after that mailin.start() has been called. */
    this.smtp = null;
};
...
Mailin.prototype.start = function (options, callback) {
    ...
    this.smtp = simplesmtp.createServer(this.options.smtpOptions);
    ...
    this.smtp.on('dataReady', function (connection, cbConnection) {
        logger.info('Processing message from ' + connection.from);

        // Store envelope addresses for later
        var envelopeFrom = connection.from,
    ...
    this.smtp.listen(_this.options.port, function (err) {
        if (!err) {
