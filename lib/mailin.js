...
Mailin.prototype.start = function (options, callback){
    ...
    options.smtpOptions = _.extend({}, this.options.smtpOptions, options.smtpOptions);
    options.smtpOptions.secure = (options.smtpOptions.secure == 'true');
    ...
    if (!options.disableWebhook) {
        request.head({
            ...
            if (err || resp.statusCode !== 200) {
                logger.warn('Webhook ' + options.webhook +
                    ' seems invalid or down. You may want to double check the webhook url.');
            } else {
                logger.info('Webhook ' + options.webhook + ' is valid, up and running.');
            }
        });
    }
    ...
    function validateAddress(addressType, email, envelope) {
        return new Promise(function (resolve, reject) {
            ...
            try {
                ...
                if (options.disableDnsValidation) {
                    validateViaLocal();
                } else {
                    validateViaDNS();
                }
    ...
    function postWebhook(connection, finalizedMessage){
        return new Promise(function (resolve, reject){
            logger.info(connection.id + ' Sending request to webhook ' + options.webhook);
            ...
            var req = request.post(options.webhook);
            ...
            req.end(function (err, resp){
                ...
                if (err || resp.statusCode !== 200) {
                    logger.error(connection.id + ' Error in posting to webhook ' + options.webhook);
                    if (resp) {
                ...
                logger.info(connection.id + ' Succesfully posted to webhook ' + options.webhook);
    ...
    function onData(stream, session, callback){
        ...
        var mailPath = path.join(options.tmp, connection.id);
    ...
    var smtpOptions = _.extend({}, options.smtpOptions || {}, {
        onData: onData,
        onAuth: onAuth,
