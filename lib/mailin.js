...
Mailin.prototype.getSubStr = function(str, delim, delimTwo) {
  var a, b;
  a = str ? str.indexOf(delim) : -1;
  if (a < 0) {
    return false;
  }
  delim = delimTwo != null ? delimTwo : delim;
  b = str.indexOf(delim, a + delim.length + 1);
  if (b < 0) {
    return str.substr(a + delim.length + 1, str.length - 1);
  } else {
    return str.substr(a + delim.length + 1, b - a - delimTwo.length - 1);
  }
};

Mailin.prototype.getTextBefore = function(str, delim) {
  var a;
  a = str ? str.indexOf(delim) : -1;
  if (a < 0) {
    return false;
...
Mailin.prototype.getTextAfter = function(str, delim) {
  var a;
  a = str ? str.indexOf(delim) : -1;
  if (a < 0) {
    return false;
