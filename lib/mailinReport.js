...
MailinReport.prototype.retrieveRawEmail = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    function fileHandler(err, data) {
      ...
      _this.rawEmail = _this.results.rawEmail = data.toString();
...
MailinReport.prototype.validateSpf = function () {
  ...
  return new Promise(function (resolve) {
    ...
    var remoteAddress = _this.connection.remoteAddress;
    var from          = _this.connection.from ||  _this.connection.envelope.mailFrom.address;
    var host          = _this.connection.host ||  _this.connection.hostNameAppearsAs;

    console.log(host);

    if(host.indexOf('[') > -1){
      host = _this._getSubStr(host, '[', ']');
    }
...
MailinReport.prototype.finalizeMessage = function () {
  ...
  return new Promise(function (resolve) {
    ...
    logger.verbose('finalizing message');
...
MailinReport.prototype._getSubStr = function(str, delim, delimTwo) {
  return str.substring(str.lastIndexOf(delim)+1,str.lastIndexOf(delimTwo));
};

MailinReport.prototype._getTextBefore = function(str, delim) {
  var a;
  a = str ? str.indexOf(delim) : -1;
  if (a < 0) {
    return false;
  } else {
...
MailinReport.prototype._getTextAfter = function(str, delim) {
  var a;
  a = str ? str.indexOf(delim) : -1;
  if (a < 0) {
    return false;
  } else {
    return str.substr(a + delim.length, str.length);
  }
};
